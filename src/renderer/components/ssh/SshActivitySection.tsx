...
const statusText: Record<BackgroundActivity['status'], string> = { connecting: '������', running: '������', completed: '�����', failed: 'ʧ��', interrupted: '���ж�', timed_out: '�ѳ�ʱ' };
...
export function SshActivitySection({ sessionId }: { sessionId: string }) {
  ...
  return <section className="relative mx-4 mt-2 mb-4 flex shrink-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-[#0b0f14] shadow-[0_8px_24px_rgba(0,0,0,0.22)]" style={{ height: collapsed ? 38 : dockHeight }} aria-label="Agent SSH �ն�">
    {!collapsed && <div onPointerDown={beginResize} className="group flex h-2 shrink-0 touch-none cursor-ns-resize items-center justify-center bg-[#111820] transition-colors hover:bg-accent/25" title="��ס�������϶������ն˸߶�"><GripHorizontal className="h-3.5 w-5 text-gray-600 transition-colors group-hover:text-accent" /></div>}
    <header className="flex h-[38px] shrink-0 items-center gap-2 border-b border-white/10 bg-[#111820] px-2 text-gray-300">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto">{terminalItems.map((terminal) => <div key={terminal.terminalId} className={`inline-flex max-w-44 shrink-0 items-center rounded ${activeTerminal?.terminalId === terminal.terminalId ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'}`}><button onClick={() => { setActiveTerminalId(terminal.terminalId); setCollapsed(false); }} className="inline-flex min-w-0 items-center gap-1.5 px-2 py-1 text-[11px]"><span className={`h-1.5 w-1.5 rounded-full ${terminal.status === 'open' ? 'bg-emerald-400' : terminal.status === 'error' ? 'bg-red-400' : 'bg-gray-500'}`} /><span className="truncate">{terminal.serverName}</span></button><button onClick={() => closeTerminal(terminal.terminalId)} className="mr-1 rounded p-0.5 text-gray-500 hover:bg-white/10 hover:text-red-300" title="�رմ��ն�"><X className="h-3 w-3" /></button></div>)}</div>
      {backgroundItems.length > 0 && <div className="group relative shrink-0"><span className="rounded px-2 py-1 text-[11px] text-gray-500">��̨ {runningBackgroundCount > 0 ? `${runningBackgroundCount} ������` : backgroundItems.length}</span><div className="invisible absolute bottom-full right-0 z-20 mb-2 w-80 rounded-md border border-white/10 bg-[#111820] p-2 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100"><div className="space-y-1">{backgroundItems.slice(0, 8).map((activity) => <div key={activity.executionId} className="flex items-center gap-2 rounded px-2 py-1 text-[11px] hover:bg-white/5"><span className="min-w-0 flex-1 truncate text-gray-300">{activity.serverName} �� {activity.command}</span><span className="shrink-0 text-gray-500">{statusText[activity.status]}</span>{['connecting', 'running'].includes(activity.status) && <button title="�жϺ�̨����" onClick={() => void window.electronAPI.ssh.cancelExecution(activity.executionId)} className="p-0.5 text-gray-500 hover:text-red-400"><Square className="h-3 w-3" /></button>}</div>)}</div></div></div>}
      <button onClick={openTerminal} className="rounded p-1 text-gray-400 hover:bg-white/10 hover:text-white" title="Ϊ��ǰ�������½������ն�"><Plus className="h-3.5 w-3.5" /></button>
      <button onClick={() => setCollapsed((value) => !value)} className="rounded p-1 text-gray-400 hover:bg-white/10 hover:text-white" title={collapsed ? 'չ���ն�' : '�۵��ն�'}>{collapsed ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button>
    </header>
